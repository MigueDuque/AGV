...
public:
      ...
      : Node("agv_controller")
  {
    ...
    twist_ctrl_subs_ = this->create_subscription<geometry_msgs::msg::Twist>(
    "/cmd_vel_ctrl", rclcpp::QoS(10),
    std::bind(&AGVController::sendCmdVel, this, std::placeholders::_1));
  ...
  void processOdom()
  {
    ...
    if (odomReceived.substr(0, ODOM_PREFIX.size()) == ODOM_PREFIX)
    {
...
private:
  ...
  void sendUART(const std::string &message)
  {
      ...
      usleep(15000); // añade un retraso de 10ms
  }
  ...
  rclcpp::Subscription<geometry_msgs::msg::Twist>::SharedPtr twist_ctrl_subs_;
